    ...
    */
    ...
    getResultFileName() {
      ...
      if (this.testRunId === Infinity) {
        do this.testRunId = Math.floor(Math.random() * 900000) + +process.pid.toString().slice(-5); while (this.fs.existsSync(getNewName()));
      }
    ...
    writeToReportOnStart(data, field) {
      ...
      if (field === this.reportUtil.jsonNames.fixture && !json.fixtures.find(el => el.name === data.name)) json.fixtures.push(data);else if (field === this.reportUtil.jsonNames.test) {
    ...
    */
    ...
    getStackTraceAsStringsArray(errs) {
      ...
      for (let index = 0; index < errs.length; index++) {
        ...
        let errName;
        if (err.errMsg) errName = err.errMsg;else if (err.message) errName = err.message;else if (err.code) {
          ...
          if (errorMsgs[err.code]) errName += `: ${errorMsgs[err.code](err)}`;else {
        ...
        if (errs[index].stack) {
          ...
          for (const stackStr of stackArr) if (isMessageSuite(stackStr)) stackTrace[index].push(stackStr);
    ...
    getStartArgObject() {
      ...
      for (let i = 0; i < args.length; i++) {
        ...
        if (argName) {
          ...
          if (argValue) obj[argName[1]] = argValue[1];else if (args[i + 1] && !argNameReg.exec(args[i + 1])) {
    ...
    prepareResultFile() {
      try {
        if (!this.appendLogs) this.fs.unlinkSync(this.getResultFileName());else if (this.fs.existsSync(this.getResultFileName())) maxTestId = this.getMaxTestId();
      } catch (e) {
        /*file doesn't exist*/
      }
    },
        ...
        if (testRunInfo.skipped) {
          ...
          for (const error of stackTrace) {
            for (let index = 0; index < error.length; index++) {
              msg += (result === this.testStatuses.broken ? this.chalk[this.chalkStyles.broken] : this.chalk.rgb(...this.chalkStyles.stackTrace))(' '.repeat(index) + error[index] + '\n');
            }
          }
    ...
    reportTaskDone(endTime, passed, warnings) {
      try {
        ...
        if (passed !== this.testsCount) summary += ', ' + this.chalk[this.chalkStyles.broken](`${this.brokenCount ? this.brokenCount : 0} ${this.testStatuses.broken}`) + ', ' + this.chalk[this.chalkStyles.failed](`${this.testsCount - passed - this.skippedCount}/${this.testsCount} ${this.testStatuses.failed}`) + ', ' + this.chalk[this.chalkStyles.skipped](`${this.skippedCount ? this.skippedCount : 0} ${this.testStatuses.skipped}`);
        ...
        if (this.appendLogs) require('child_process').execSync(`npx acd-html-combine ${path.dirname(this.getResultFileName())} --dest ${reportPath}`);else if (this.isSaveAsFile) this.reportUtil.generateReportAsHtml();else this.reportUtil.generateReport();
