...
if (oldVersions.some(v => testsContent.includes(v))) fs.writeFileSync(testsFile, oldVersions.reduce((val, ver) => val.replace(ver, v1314Replaced), testsContent));
...
if (concurencyContent.includes(concurencyBlockStr) || concurencyContent.includes(featureCheckText)) fs.writeFileSync(concurencyBlockFile, concurencyContent.replace(concurencyBlockStr, concurencyBlockReplace).replace(featureCheckText, featureCheckReplace));