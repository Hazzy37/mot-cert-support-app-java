...
module.exports = {
  ...
  getOriginalReportPath: function () {
    ...
    if (!fs.existsSync(originalReportPath)) return 'node_modules/testcafe-reporter-acd-html-reporter/report';
  ...
  generateReportAsHtml: function (...args) {
    ...
    const generatedReport = html.replace('<div class="tests-tree"></div>', `<div class="tests-tree">${this.getJsonAsHtml(json)}</div>`).replace('startTime', json.startTime).replace(/<head>.*<\/head>/gs, htmlHead);
  ...
  getJsonAsHtml: function (json) {
    ...
    stepsArray.forEach(stepsData => {
      generatedReport += stepsData.steps.replace('<step>', `<div fixtureId="${stepsData.id}" screenshot="${stepsData.screenshot}" durationMs="${stepsData.durationMs}" userAgent="${stepsData.userAgent}" time="${stepsData.time}" f="${stepsData.fixture.replace(/([\t\n\f />"'=]+)/, '')}" t="${stepsData.test.replace(/([\t\n\f />"'=]+)/, '')}">`);
      if (stepsData.stackTrace && stepsData.stackTrace.length) generatedReport += `<div traceId="${stepsData.id}">${JSON.stringify(stepsData.stackTrace)}</div>`;
    });
    generatedReport += '</div>';
  ...
  stepsToString: function (steps) {
    ...
    steps.forEach(step => {
      ...
      if (step.name) stepsString += `<div class="stepName" onclick="stepOnClick(this)">${step.name}</div>`;
